...
let solve1 data =
  ...
  let (w, h) =
    ...
    (int_of_string (List.nth parts 0), int_of_string (List.nth parts 1)) in
  let robots = List.map parse_line (List.tl data) in
  let new_pos = List.fold_left (fun acc _ -> List.map (move_robot w h) acc) robots (List.init 100 (fun _ -> 0)) in
...
let print_board w h robots =
  ...
  Array.iter (fun row -> Printf.printf "%s\n" (String.of_seq (Array.to_seq row))) board
...
let solve2 data =
  ...
  let _ = List.fold_left (fun acc step ->
    ...
    new_pos
  ) robots (List.init 10000 (fun i -> i)) in
  ()
