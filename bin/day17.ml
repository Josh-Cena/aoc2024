...
let exec reg1_i reg2_i reg3_i prog =
  ...
  let step () =
    if !pc >= Array.length prog then false
      ...
      | 5 ->
          (* out *)
          output := (get_combo op mod 8) :: !output;
          pc := !pc + 2
      | 6 ->
          (* bdv *)
          reg2 := !reg1 asr get_combo op;
          pc := !pc + 2
      | 7 ->
          (* cdv *)
          reg3 := !reg1 asr get_combo op;
          pc := !pc + 2
...
let xor_bits bits1 bits2 = List.map2 xor_bit bits1 bits2
...
let is_bit_compatible existing num info =
  ...
  | Symbol (sign, ind) ->
      let existing_bit =
        if ind >= Array.length existing then 0 else existing.(ind)
      in
