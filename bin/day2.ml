...
let solve1 data =
  let parse_line line = List.map int_of_string (String.split_on_char ' ' line) in
  let rec differences lst = match lst with
    | a :: b :: rest -> get_diff_type (b - a) :: differences (b :: rest)
    | _ -> [] in
  let is_safe lst =
    let diffs = differences lst in
    (List.for_all is_inc diffs) || (List.for_all is_dec diffs) in
...
let solve2 data =
  ...
  let diff_groups diffs =
    let rec aux diffs (inc, dec, invalid) = match diffs with
      ...
      | [] -> (inc, dec, invalid) in
    aux (List.mapi (fun i x -> (i, x)) diffs) ([], [], []) in
  let is_safe lst =
    let diffs = differences lst in
    ...
    | (_, [], [(j, dj); (i, di)]) | (_, [(j, dj)], [(i, di)]) when j = i + 1 || j = i - 1 ->
      is_inc (get_diff_type (di + dj))
    | ([], _, [(j, dj); (i, di)]) | ([(j, dj)], _, [(i, di)]) when j = i + 1 || j = i - 1 ->
