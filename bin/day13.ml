...
let rec parse_lines lines prize_shift =
  ...
  | l1 :: l2 :: l3 :: rest ->
    ...
    let prize = (of_string (Re.Group.get m3 1) + prize_shift, of_string (Re.Group.get m3 2) + prize_shift) in
    (mat, prize) :: begin match rest with
    | [] -> []
...
let solve_lp mat (prize_x, prize_y) =
  ...
  | Some inv_mat ->
    ...
    if x >= of_int 0 && y >= of_int 0 && Z.equal x.den (Z.of_int 1) && Z.equal y.den (Z.of_int 1) then
      Some (x, y)
    else
      None
...
let solve1 data =
  ...
  let solutions = List.filter_map (fun (mat, prize) -> solve_lp mat prize) blocks in
  let total = List.map (fun (x, y) -> (of_int 3) * x + y) solutions |> List.fold_left (+) (of_int 0) in
  Printf.printf "%s\n" (to_string total)
