  ...
  try
    while true do
      ...
      if new_x < 0 || new_x >= width || new_y < 0 || new_y >= height then
        raise Exit
      else if !mat.(new_y).(new_x) = '#' then direction := turn_right !direction
      else (
        !mat.(new_y).(new_x) <- 'X';
        pos := (new_x, new_y))
    done
  ...
  let covered =
    Array.fold_left
      (fun acc row ->
        acc
        + Array.fold_left
            (fun acc cell -> if cell = 'X' then acc + 1 else acc)
            0 row)
      0 !mat
  in
  Printf.printf "%d\n" covered
...
let solve2 data =
    ...
    try
      while true do
        ...
        if new_x < 0 || new_x >= width || new_y < 0 || new_y >= height then
          ...
          pos := (new_x, new_y);
          !path.(new_y).(new_x) <- !direction :: !path.(new_y).(new_x))
      done
    with Exit b -> b
  in
  let all_x = ref [] in
  for y = 0 to height - 1 do
    for x = 0 to width - 1 do
      if !mat.(y).(x) = 'X' then all_x := (x, y) :: !all_x
    done
  done;
