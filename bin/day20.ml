  ...
  let start_pos =
    Option.get
      (Array.find_mapi
         (fun i row ->
           Array.find_mapi
             (fun j c -> if c = 'S' then Some (i, j) else None)
             row)
         mat)
  in
...
let solve1 data =
  ...
  let cheats =
    List.map (fun (i, j) -> num_working_cheats distances i j 2) path
  in
...
let solve2 data =
  let distances, path = parse_input data in
  let cheats =
    List.map (fun (i, j) -> num_working_cheats distances i j 20) path
  in
  Printf.printf "%d\n" (List.fold_left ( + ) 0 cheats)
