...
let numpad = Hashtbl.of_seq (List.to_seq [
  ('7', (-2, 3));
  ('8', (-1, 3));
  ('9', (0, 3));
  ('4', (-2, 2));
...
let dirpad = Hashtbl.of_seq (List.to_seq [
  ('^', (-1, 0));
  ('A', (0, 0));
  ('<', (-2, -1));
  ('v', (-1, -1));
...
let optimal_path (from_x, from_y) (to_x, to_y) =
  ...
  if dx = 0 || dy = 0 then move_seq dx dy @ ['A'] else
  ...
  else if dx < 0 then move_seq dx 0 @ move_seq 0 dy @ ['A'] (* LD, LU *) else
  move_seq 0 dy @ move_seq dx 0 @ ['A'] (* DR, UR *)
...
let rec optimal_path_cost from_pos to_pos depth memo =
  ...
  else let path = optimal_path from_pos to_pos in
  let total_cost = if depth = 1 then List.length path else
    optimal_total_path_cost ((List.map (fun d -> Hashtbl.find dirpad d) ('A' :: path))) (depth - 1) memo
  in
  Hashtbl.add memo (from_pos, to_pos, depth) total_cost;
  total_cost
and
optimal_total_path_cost poses depth memo =
  let rec aux acc = function
  | [] | [_] -> acc
  | step1 :: step2 :: rest ->
    let step_cost =
      optimal_path_cost step1 step2 depth memo in
    aux (acc + step_cost) (step2 :: rest)
  in aux 0 poses

let solve1 data =
  let memo = Hashtbl.create 100 in
  let total = List.fold_left (fun acc line ->
    let chars = List.of_seq (String.to_seq line) in
    let positions = List.map (fun c -> Hashtbl.find numpad c) ('A' :: chars) in
    let cost = optimal_total_path_cost positions 3 memo in
    let numeric = int_of_string (String.sub line 0 (String.length line - 1)) in
    acc + (cost * numeric)
  ) 0 data in
  Printf.printf "%d\n" total
