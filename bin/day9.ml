let solve1 data =
  ...
  try
    for i = Array.length chunks - 1 downto 0 do
      if List.hd !blanks >= i then raise Exit
...
let solve2 data =
  ...
  let build_segments chunks =
    let rec aux chunks acc ind =
      ...
      | occ :: [] ->
          ...
          id := !id + 1;
          let seg = { chunks; total_size = occ; free_size = 0; ind } in
          List.rev (seg :: acc)
      | occ :: free :: t ->
          let chunks = [ { id = !id; occ; ind } ] in
          id := !id + 1;
          let seg =
            { chunks; total_size = occ + free; free_size = free; ind }
          in
    ...
    let rec move_all_chunks chunks =
      ...
      | { id; occ; _ } :: t ->
          ...
          | Some p when p < i ->
              ...
              let new_ind =
                match target_seg.chunks with
                | [] -> target_seg.ind
                | hd :: _ -> hd.ind + hd.occ
              in
              (* Update target segment *)
              let new_target_chunks =
                { id; occ; ind = new_ind } :: target_seg.chunks
              in
              let new_target_free = target_seg.free_size - occ in
              !segments.(p) <-
                {
                  target_seg with
                  chunks = new_target_chunks;
                  free_size = new_target_free;
                };
              (* Update source segment *)
              let new_source_chunks =
                List.filter (fun c -> c.id <> id) seg.chunks
              in
  ...
  let checksum_of_chunk chunk =
    List.fold_left ( + ) 0
      (List.init chunk.occ (fun i -> (chunk.ind + i) * chunk.id))
  in
  let checksum_of_segment seg =
    List.map checksum_of_chunk seg.chunks |> List.fold_left ( + ) 0
  in
  let result =
