...
let rec string_of_tree node =
  ...
  | Input name -> name
  | And (_, x, y) ->
      Printf.sprintf "(%s & %s)" (string_of_tree x) (string_of_tree y)
  | Or (_, x, y) ->
      Printf.sprintf "(%s | %s)" (string_of_tree x) (string_of_tree y)
  | Xor (_, x, y) ->
      Printf.sprintf "(%s ^ %s)" (string_of_tree x) (string_of_tree y)
...
let rec make_tree name (inputs, circuit) =
  if Hashtbl.mem inputs name then Input name
  else
    ...
    | "AND" ->
        ...
        if String.compare lstr rstr < 0 then And (name, l, r)
        else And (name, r, l)
    | "OR" ->
        let l = make_tree x (inputs, circuit) in
        let r = make_tree y (inputs, circuit) in
    ...
    | "XOR" ->
        ...
        let lstr = string_of_tree l in
        let rstr = string_of_tree r in
        if String.compare lstr rstr < 0 then Xor (name, l, r)
        else Xor (name, r, l)
...
let make_trees data swaps =
  ...
  ( inputs,
    Hashtbl.fold
      (fun k _ acc ->
        if String.starts_with ~prefix:"z" k then k :: acc else acc)
      circuit []
...
let solve2 data =
  ...
  let find_output_with_reparation op1 op op2 =
    begin match
      Hashtbl.find_opt op_to_id (Printf.sprintf "%s %s %s" op1 op op2)
    with
    ...
    | None ->
        let op2_swap =
          find_key_by_left (Printf.sprintf "%s %s " op1 op) op_to_id
        in
        let op1_swap =
          find_key_by_right (Printf.sprintf " %s %s" op op2) op_to_id
        in
        begin match (op2_swap, op1_swap) with
        | Some (op2_swap, _), Some (op1_swap, _) ->
            (* Luckily didn't happen *)
            failwith
              (Printf.sprintf "I have two candidates: %s,%s and %s,%s" op2_swap
                 op1_swap op1_swap op2_swap)
        | Some (op2_swap, _), None ->
            failwith
              (Printf.sprintf "Found swap: %s,%s add to swaps to continue\n" op2
                 op2_swap)
        ...
        | None, None ->
            (* Luckily didn't happen *)
            failwith
              (Printf.sprintf "Couldn't find any reparation for %s %s %s" op1 op
                 op2)
        end
    end
  in
  let _ =
    List.fold_right
      (fun tree last_carry ->
        ...
        if int_of_string ind = List.length trees - 1 then "DONE"
        else
       ...
       foo and bar themselves may be wrong though! *)
          ...
          if name = "z00" then begin
            ...
            let _output_id =
              find_output_with_reparation last_carry "^" foo_id
            in
            carry_id)
      trees "N/A"
  in
