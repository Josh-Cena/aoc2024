let counts list =
  let rec aux acc = function
    | [] -> acc
    | x :: xs ->
        let count = List.fold_left (fun c y -> if y = x then c + 1 else c) 0 xs in
...
let solve n data =
  ...
  let rec evolve n nums =
    if n = 0 then nums
      ...
      let new_nums = List.fold_left (fun acc (x, count) ->
        let sx = string_of_int x in
        let new_x = if x = 0 then [1]
        else if String.length sx mod 2 = 0 then
          [int_of_string (String.sub sx 0 (String.length sx / 2));
           int_of_string (String.sub sx (String.length sx / 2) (String.length sx / 2))]
        else
          [x * 2024] in
        List.fold_left (fun acc' nx -> match List.assoc_opt nx acc' with
