  ...
  let rec sort_list orders lst =
    ...
    | (a, b) :: rest ->
      ...
      | Some i, Some j when i > j ->
        let lst' = List.mapi (fun k x -> if k = i then b else if k = j then a else x) lst in
...
let solve2 data =
  ...
  let lists = List.map (List.map int_of_string >> String.split_on_char ',') (List.tl (List.drop_while (fun x -> String.contains x '|') data)) in
  let invalid_lists = List.filter (fun lst -> not (List.for_all (order_holds (num_to_ind lst)) orders)) lists in
