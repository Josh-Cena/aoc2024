...
let rec rep f n acc =
  if n = 0 then acc
  else rep f (n - 1) (f acc)

let rec rep_history f n acc =
  if n = 0 then [acc]
  else acc :: rep_history f (n - 1) (f acc)
...
let solve2 data =
  ...
  let total_gain = Hashtbl.create (List.length diffs * List.length (List.hd diffs)) in
  List.iter (fun diff_seq ->
    let seq_gain = Hashtbl.create (List.length diff_seq) in
    List.iter (fun ((d1, _), (d2, _), (d3, _), (d4, p4)) ->
      let key = (d1, d2, d3, d4) in
      if not (Hashtbl.mem seq_gain key) then
        Hashtbl.add seq_gain key p4
    ) (windows4 diff_seq);
    Hashtbl.iter (fun key gain ->
      let sum = (Option.fold ~none:gain ~some:((+) gain) (Hashtbl.find_opt total_gain key)) in
