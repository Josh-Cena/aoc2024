...
let solve1 data =
  ...
  let rec do_move r c dr dc =
    ...
    (!mat).(r).(c) <- '.'
  in
  let start_r = Option.get (Array.find_index (fun x -> Array.exists ((=) '@') x) !mat) in
  let start_c = Option.get (Array.find_index ((=) '@') (!mat).(start_r)) in
  let pos = ref (start_r, start_c) in
  let dirs = List.map char_to_dir (List.of_seq (String.to_seq (List.nth parts 1))) in
  List.iter (fun (dr, dc) ->
    let (r, c) = !pos in
...
let solve2 data =
  ...
  let mat = List.hd parts
    |> String.split_on_char '\n'
    |> List.map (fun line -> line |> String.to_seq |> Seq.flat_map (fun c ->
        match c with
        | '#' -> List.to_seq ['#'; '#']
        | 'O' -> List.to_seq ['['; ']']
        | '.' -> List.to_seq ['.'; '.']
